...
func (s *Service) Upsert(r Record) error {
	...
	if r.ID == "" || r.Yard == "" {
		return fmt.Errorf("classification: id and yard required")
	}
...
func (s *Service) Transition(id, from, to string) error {
	...
	r, ok := s.records[id]
	if !ok {
		return fmt.Errorf("classification: record %s missing", id)
	}
	if from != "" && r.Status != from {
		return fmt.Errorf("classification: expected %s got %s", from, r.Status)
	}
	if strings.TrimSpace(to) == "" {
		return fmt.Errorf("classification: empty target status")
	}
	r.Status = to
	r.UpdatedAt = time.Now().UTC()
	s.records[id] = r
