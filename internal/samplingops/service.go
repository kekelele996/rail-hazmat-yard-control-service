...
func (s *Service) Upsert(r Record) error {
	...
	if r.ID == "" || r.Yard == "" {
		return fmt.Errorf("samplingops: id and yard required")
	}
...
func (s *Service) Transition(id, from, to string) error {
	...
	r, ok := s.records[id]
	if !ok {
		return fmt.Errorf("samplingops: record %s missing", id)
	}
	if from != "" && r.Status != from {
		return fmt.Errorf("samplingops: expected %s got %s", from, r.Status)
	}
	if strings.TrimSpace(to) == "" {
		return fmt.Errorf("samplingops: empty target status")
	}
	r.Status = to
	r.UpdatedAt = time.Now().UTC()
	s.records[id] = r
